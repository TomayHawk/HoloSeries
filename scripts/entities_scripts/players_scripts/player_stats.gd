...
# TODO: currently ignores any states, action_fail_count
func basic_attack(initialization: bool = false) -> void:
	...
	if initialization:
		...
		action_area.get_node(^"CollisionShape2D").shape.radius = 1000.0
	...
	# TODO: need to deal with large fail count
	# action fails for allies with no action target
