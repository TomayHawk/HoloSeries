...
func ally_process() -> void:
	...
	# if ally in combat
	if player_node.ally_in_attack_range and distance_to_main_player < 250:
		# pause movement
	...
	# if ally can move
		...
		# check for obstacles
		while true:
			player_node.get_node(^"ObstacleCheck").set_target_position(target_direction * 8)
			...
			if player_node.get_node(^"ObstacleCheck").is_colliding():
				...
				if possible_directions.is_empty():
					player_node.update_velocity(Vector2.ZERO)
					player_node.set_move_direction(VECTOR_TO_DIRECTION[snap_vector])
					player_node.update_velocity(snap_vector)
					break
...
'''
	...
	var snap_vector: Vector2 = player_node.snap_vector(target_direction)

	# check for obstacles
	while true:
		player_node.get_node(^"ObstacleCheck").set_target_position(snap_vector * 8)
		player_node.get_node(^"ObstacleCheck").force_shapecast_update()
...
func _on_attack_area_body_exited(body: Node2D) -> void:
	if get_parent() is PlayerBase:
		...
		if get_parent().enemy_nodes_in_attack_area.is_empty():
			get_parent().ally_in_attack_range = false

func _on_attack_area_body_entered(body: Node2D) -> void:
	if get_parent() is PlayerBase:
		get_parent().enemy_nodes_in_attack_area.push_back(body)
		get_parent().ally_in_attack_range = true
