...
var current_face_direction := direction_states.UP:
	set(next_direction):
		...
		animation_index = current_face_direction + 4
		if !player_stats_node.alive: return
		if current_attack_state == attack_states.ATTACK: animation_index += 4
...
var current_move_direction_2 := direction_states.UP:
	set(next_direction):
		...
		if !player_stats_node.alive: return
		current_face_direction = move_directions[current_move_direction_2][0]

var move_directions := {
	direction_states.UP: [direction_states.UP, Vector2(0, -1)],
	direction_states.DOWN: [direction_states.DOWN, Vector2(0, 1)],
	direction_states.LEFT: [direction_states.LEFT, Vector2(-1, 0)],
	direction_states.RIGHT: [direction_states.RIGHT, Vector2(1, 0)],
	direction_states.UP_LEFT: [direction_states.LEFT, Vector2(-1, -1).normalized()],
	direction_states.UP_RIGHT: [direction_states.LEFT, Vector2(1, -1).normalized()],
	direction_states.DOWN_LEFT: [direction_states.RIGHT, Vector2(-1, 1).normalized()],
	direction_states.DOWN_RIGHT: [direction_states.RIGHT, Vector2(1, 1).normalized()]
}
...
# player node information variablesw
...
func _physics_process(delta):
	...
	if velocity == Vector2.ZERO: current_move_state = move_states.IDLE
		...
		match round(velocity):
			Vector2(0, -1): current_move_direction_2 = direction_states.UP
			Vector2(0, 1): current_move_direction_2 = direction_states.DOWN
			Vector2(-1, 0): current_move_direction_2 = direction_states.LEFT
			Vector2(1, 0): current_move_direction_2 = direction_states.RIGHT
			Vector2(-1, -1): current_move_direction_2 = direction_states.UP_LEFT
			Vector2(1, -1): current_move_direction_2 = direction_states.UP_RIGHT
			Vector2(-1, 1): current_move_direction_2 = direction_states.DOWN_LEFT
			Vector2(1, 1): current_move_direction_2 = direction_states.DOWN_RIGHT
	...
	match current_move_state:
		...
		move_states.WALK:
			pass
		move_states.DASH:
			velocity += move_directions[current_move_direction_2][1] * dash_speed * delta * \
...
func _input(_event):
	...
	if current_move_state == move_states.DASH && Input.is_action_just_released("dash"):
		if velocity != Vector2.ZERO:
			current_move_state = move_states.WALK
		else:
			current_move_state = move_states.IDLE
